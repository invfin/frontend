...
const { pending, data, error, execute, refresh } = await useFetch("https://example.com:8000/api/v1/blogs/", {
  query: query,
  server: false,
  lazy: true,
...
<template>
  <div>
    <BlogsFilters v-model:modelValue="filters" class="mb-4 mt-4" @update:modelValue="handleFilters" />
    <div class="grid grid-cols-3 gap-4 mt-4">
      ...
      <BlogsEntry v-else v-for="entry in entries" :entry="entry" />
    </div>
